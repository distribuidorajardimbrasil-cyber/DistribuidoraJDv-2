        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-zinc-500 dark:text-zinc-400">Acompanhe o histórico de entregas da equipe.</p>
                </div>
            </div>

            <div className="bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm dark:shadow-none flex flex-col lg:flex-row gap-4 lg:items-center justify-between">
                <div className="flex flex-wrap items-center gap-4">
                    <div className="flex items-center gap-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-100 dark:border-zinc-800/50 p-2 px-3 rounded-xl">
                        ...
                        <select
                            ...
                            className="bg-transparent border-none outline-none font-bold text-zinc-700 dark:text-zinc-300 cursor-pointer text-sm w-40 truncate"
                        >
                ...
                <div className="flex items-center gap-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-100 dark:border-zinc-800/50 p-2 px-3 rounded-xl ml-auto border-dashed border-emerald-300">
                    <Filter size={18} className="text-emerald-500" />
                    <select
                        value={filterCategory}
                        onChange={(e) => setFilterCategory(e.target.value)}
                        className="bg-transparent border-none outline-none font-bold text-emerald-700 dark:text-emerald-400 cursor-pointer text-sm w-32 truncate"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm dark:shadow-none">
                    ...
                    <p className="text-3xl font-black text-zinc-800 dark:text-zinc-200">{summary.totalOrders}</p>
                </div>
                <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm dark:shadow-none">
                    <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-1">Itens Entregues</p>
                    <p className="text-3xl font-black text-indigo-600 dark:text-indigo-400">{summary.totalItems} un.</p>
                </div>
                <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm dark:shadow-none">
                    <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-1">Valor Representado</p>
                    <p className="text-3xl font-black text-emerald-600 dark:text-emerald-400">{formatCurrency(summary.totalValue)}</p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-sm dark:shadow-none p-12 text-center text-zinc-500 dark:text-zinc-400 font-medium">
                ...
                <div className="space-y-4">
                    {orders.map(order => (
                        <div key={order.id} className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm dark:shadow-none hover:border-emerald-200 transition-colors">
                            <div
                                className="flex items-center justify-between cursor-pointer"
                                onClick={() => setExpandedOrderId(expandedOrderId === order.id ? null : order.id)}
                            >
                                <div className="flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-xl bg-zinc-100 dark:bg-zinc-800/50 flex items-center justify-center text-zinc-500 dark:text-zinc-400">
                                        <Package size={24} />
                                    </div>
                                    <div>
                                        <div className="flex items-center gap-2">
                                            <p className="font-bold text-lg">Pedido #{order.id}</p>
                                            <span className="text-xs font-bold px-2 py-0.5 rounded-md bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400">
                                ...
                                <div className="flex items-center gap-4">
                                    <div className="text-right hidden sm:block">
                                        <p className="font-bold text-emerald-600 dark:text-emerald-400">{formatCurrency(order.total_amount)}</p>
                            ...
                            {expandedOrderId === order.id && (
                                <div className="mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-800/50 grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-2">
                                    <div className="bg-zinc-50 dark:bg-zinc-950 p-4 rounded-xl border border-zinc-100 dark:border-zinc-800/50">
                                        ...
                                        <p className="font-bold text-zinc-800 dark:text-zinc-200 mb-1">{order.customer_name}</p>
                                            ...
                                            {order.items.map((item, idx) => (
                                                <li key={idx} className="flex justify-between text-sm">
                                                    <span className="text-zinc-600 dark:text-zinc-400">
                                                        <span className="font-bold text-zinc-900 dark:text-zinc-50 mr-2">{item.quantity}x</span>
